...
@Component({
  selector: 'app-packages-by-categories',
  templateUrl: './packages-by-categories.component.html',
  styleUrl: './packages-by-categories.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PackagesByCategoriesComponent implements OnInit, OnDestroy {
  step: 'detail' | 'signup' | 'payment' | 'provider card' | 'providers' | 'confirmation' = 'detail';
  card: any;
  bundles: any[] = [];
  providers: any[] = [];
  ...
  ngOnInit(): void {
    this.route.params.subscribe((params) => {
      ...
      const parsedBundleId = Number(params['bundleId']);
      this.bundleId = Number.isFinite(parsedBundleId) && parsedBundleId > 0 ? parsedBundleId : null;

      if (this.facilityId && this.categoryId) {
        if (this.bundleId) {
          this.loadSingleFacilityBundle(this.facilityId, this.bundleId);
        } else {
          this.loadBundlesByCategory(this.facilityId, this.categoryId);
        }
      }
    });
  ...
  private loadBundlesByCategory(facilityId: number, categoryId: number): void {
    this.generalService.getBundlesByCategoryId(facilityId, categoryId).subscribe(
      (response) => {
  ...
  private loadSingleFacilityBundle(facilityId: number, bundleId: number): void {
    this.generalService
      .commonGet(`UnAuthorize/getFacilityBundleDetail?facilityId=${facilityId}&bundleId=${bundleId}`)
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          this.bundles = [];
          this.cdr.markForCheck();
          console.error('Error fetching facility bundle detail:', error);
        }
      });
  }
    ...
    this.generalService.savePatient(payload).subscribe(
      (response) => {
        if (response.status == 0) {
          ...
          this.isFormSubmitting = false;
          this.cdr.markForCheck();
          this.generalService.getAllProviders(this.facilityId, this.categoryId).subscribe((providersRes) => {
  ...
  loadSlotsForDate(formattedDate: string): void {
    ...
    this.generalService
      .getProviderScheduledSlots(formattedDate, this.categoryId, this.facilityId, this.selectedProvider.providerId)
      .subscribe(
        (response) => {
  ...
  private initStripePaymentIntent(): void {
    ...
    paymentIntent$.subscribe({
      next: (res) => {
        ...
        const clientSecret =
          normalizedClientSecret.length > 0 && normalizedClientSecret.includes('_secret_')
        ...
        if (responseAccountId) this.stripeConnectedAccountId = responseAccountId;
        if (responsePublishableKey) this.stripePublishableKey = responsePublishableKey;
  ...
  private mountStripePaymentElement(clientSecret: string): void {
    if (!clientSecret || this.paymentProvider !== 'stripe') return;

    if (!this.configureStripeClientContext()) return;
    ...
    this.stripeService
      ...
      .subscribe({
        next: (elements) => {
          ...
          this.stripePaymentElementInstance.mount('#packages-by-categories-stripe-payment-element');
          this.stripeErrorMessage = null;
          this.cdr.markForCheck();
        },
        error: () => {
  ...
  private configureStripeClientContext(): boolean {
    ...
    if (this.stripeContextSignature === signature) return true;
  ...
  private buildStripeReturnUrl(): string {
    const basePath = this.router.url.split('?')[0] || `/packages-by-categories/${this.facilityId}`;
  ...
  private storeStripeContext(): void {
    ...
    sessionStorage.setItem('packagesByCategoriesStripeContext', JSON.stringify(context));
  }

  private restoreStripeContext(): void {
    const raw = sessionStorage.getItem('packagesByCategoriesStripeContext');
    if (!raw) return;
    ...
    try {
      ...
      sessionStorage.removeItem('packagesByCategoriesStripeContext');
    } catch {}
  }
