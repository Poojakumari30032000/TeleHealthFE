...
@Component({
  selector: 'app-buy-new-treatment',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './buy-new-treatment.component.html',
  styleUrl: './buy-new-treatment.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class BuyNewTreatmentComponent implements OnInit, OnDestroy {
  ...
  stripeConfirming: boolean = false;
  ...
  card: any = null;
  isSquareReady: boolean = false;
  isSquareInitializing: boolean = false;
  ...
  patientId: any = this.auth.getPatientId();
  currency: string = 'usd';
    ...
    this.generalService.getSquareAppIdByFacilityId(this.facilityId).subscribe({
      next: (res) => {
        ...
        if (this.applicationID && this.locationID) {
          ...
          this.paymentProvider = 'none';
          this.paymentProviderMessage = this.paymentProviderMessage || 'Payment is currently unavailable. Please try again later.';
        }
        this.paymentProviderLoading = false;
        this.cdr.markForCheck();
      },
      error: () => {
        ...
        this.locationID = '';
        this.paymentProvider = 'none';
        this.squareErrorMessage = 'Payment configuration is not available.';
        this.paymentProviderMessage = this.paymentProviderMessage || 'Payment is currently unavailable. Please try again later.';
        this.paymentProviderLoading = false;
        this.cdr.markForCheck();
      },
  ...
  async payWithSquare(): Promise<void> {
  ...
  private payWithSavedCard(): void {
    ...
    this.generalService.CreatePaymentAndAppointmentWithSavedCard(payload).subscribe({
      next: (response) => {
  ...
  private async payWithNewCard(): Promise<void> {
  ...
  private handleStripeStatus(status?: string): void {
    ...
    if (normalized === 'succeeded') {
      if (this.selectedSlot?.providerScheduledSlotId) {
        this.setStripeReturnState('processing', 'Finalizing appointment confirmation...');
        this.step = 'confirmation';
        this.saveAppointment();
      } else {
        this.setStripeReturnState('succeeded', 'Payment successful.');
        this.step = 'confirmation';
        this.openQuestionnaireModal();
        this.isFormSubmitting = false;
        this.stripeConfirming = false;
    ...
    this.stripeService
      ...
      .subscribe({
        next: (elements) => {
          ...
          this.stripePaymentElementInstance.mount('#buy-treatment-stripe-payment-element');
          this.stripeErrorMessage = null;
          this.cdr.markForCheck();
        },
        error: () => {
  ...
  private buildStripeReturnUrl(): string {
    const basePath = this.router.url.split('?')[0] || '/buy-new-treatment';
  ...
  private storeStripeContext(): void {
    ...
    sessionStorage.setItem('stripeCheckoutContext', JSON.stringify(context));
  }

  private restoreStripeContext(): void {
    const raw = sessionStorage.getItem('stripeCheckoutContext');
    if (!raw) return;
    try {
      ...
      sessionStorage.removeItem('stripeCheckoutContext');
    } catch {}
  }
  ...
  saveAppointment(): void {
    ...
    if (this.paymentProvider === 'stripe' && (this.lastStripePaymentIntentId || this.lastStripeSetupIntentId)) {
      ...
      if (uid != null) stripePayload.userId = uid;
  ...
  isPayNowDisabled(): boolean {
    ...
    if (this.paymentProvider === 'square') {
      if (this.paymentMethod === 'new') return !this.isSquareReady || !this.authorizeRecurringCardCharge;
      return false;
    }
    return true;
  }
  ...
  private initSquareCard(): void {
    ...
    if (!this.applicationID || !this.locationID) {
      this.squareErrorMessage = 'Payment configuration is not available. Please try again later.';
      this.isSquareReady = false;
      this.card = null;
      this.cdr.markForCheck();
      return;
    ...
    setTimeout(async () => {
      try {
        ...
        this.squareErrorMessage = 'Unable to load card input. Please try again.';
        this.isSquareReady = false;
        this.card = null;
      } finally {
        this.isSquareInitializing = false;
  ...
  private openQuestionnaireModal(): void {
  ...
  closeQuestForm(): void {
    ...
    this.renderQuestionnaire = false;
    this.cdr.markForCheck();
  }
